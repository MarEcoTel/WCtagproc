...
mark.surfacings <- function(data, lag = 15) { #first, internal, terminal, unknown, only surfacing
  ...
  for (w in unique(data$TagID)) {
    ...
    deeps <- which(whale$Kmeans %in% 'Deep')
    ...
    for (i in 1:nrow(whale)) {
      ...
      if (i < deeps[1]) {
        firstrows <- whale[which(whale$Event %in% 'Surface' & whale$RecordNo < deeps[1]),]
        if (nrow(firstrows) > 1) {
    ...
    #internal surfacings
    internals <- which(is.na(whale$SurfaceType) & whale$Event %in% 'Surface')
    first <- TRUE
    for (i in 1:nrow(whale)) {
