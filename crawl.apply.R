...
#' @param data A dataframe containing locations from one tag to be used by the model
...
#' @param prefilter Default is FALSE. TRUE if you want to use pre-filtering to remove locations with fast speeds between them greater than 7.5 m/s.
...
crawl.apply <- function(data, model.interval = '1 hour', crs = 2230, land.adjust = NULL, img.path = NULL, prefilter = FALSE, ...) {
  if (all(is.na(data$Error.radius))) {
    ...
    for (i in 1:nrow(sf_locs)) {
      sf_locs$fixpar[i] <- list(c(1, 1, NA, NA))
      sf_locs$constr[i] <- list(list(lower = c(-Inf, -4), upper = (c(Inf, 4))))
    }
