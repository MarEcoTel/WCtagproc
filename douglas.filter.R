...
douglas.filter <- function(argos, argos_method, method, keep_lc = NULL, maxredun = NULL, duplrec = 'offset', #inputs required by all filtering methods
  ...
  outliers <- data.frame()
  argos <- data.frame(argos)

  #convert dates and times into class POSIXct datetime and convert lc to characters
  if ('POSIXct' %in% class(argos$Date) == FALSE) {
    dates <- suppressWarnings(lubridate::ymd_hms(argos$Date))
    if (all(is.na(dates))) {
      dates <- suppressWarnings(lubridate::mdy_hms(argos$Date))
      if (all(is.na(dates))) {
        dates <- suppressWarnings(lubridate::dmy_hms(argos$Date))
      }
    }
    argos$Date <- dates
  }
  ...
  if (duplrec == 'offset') {
    ...
    if (duplrec == 'filter') {
      ...
      outliers <- rbind(data.frame(outliers), argos[which(argos$outlier == TRUE),])
  ...
  all <- rbind(data.frame(outliers), retained)
